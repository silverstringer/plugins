...
namespace plugin {
    ...
    public:
        CSVWriter(string directory = string(getenv("HOME")) + "/plugins", string delm = ";", bool write = false) :
    ...
    using VBuffer = vector<vector<string>>;
    class CSVRead {
        char delimeter;
        string filename;
    public:
    ...
    static VBuffer read_csv(string filename) {
        VBuffer result;
    ...
    static vector<map<string, string>> read_csv_map(string filename) {
        ...
        for (int i = 1; i < csv.size(); i++) {
            ...
            for (int j = 0; j < csv[i].size(); j++) {
                //check column name
                if (j >= names.size())
                    std::cerr << "no name for column " + std::to_string(j) + " line " + std::to_string(i) + " file " +
                                 filename;
    ...
    bool compressFile(string filename) {
        ...
        if (pathObj.has_extension()) {
            if (pathObj.extension().string() ==
                ".csv" /*&& !bfs::exists(bfs::change_extension(pathObj, "gzip").string())*/) {
                if (pathObj.extension().string() == ".gzip")
                    return false;

                std::ifstream inStream(filename, std::ios_base::in | std::ios_base::binary);
                std::ofstream outStream(bfs::change_extension(pathObj, "gzip").string(),
    ...
    bool decompressFile(string filename) {
        ...
        if (pathObj.has_extension()) {
            if (pathObj.extension().string() ==
                ".gzip" /*&& !bfs::exists(bfs::change_extension(pathObj, "gzip").string())*/) {
                if (pathObj.extension().string() == ".csv")
                    return false;

                std::ifstream inStream(filename, std::ios_base::in | std::ios_base::binary);
                std::ofstream outStream(bfs::change_extension(pathObj, "csv").string(),
                                        std::ios_base::out | std::ios_base::binary);
                bio::filtering_streambuf<bio::input> in;
    ...
    std::vector<bfs::directory_entry> listFileinDir(string path = string(getenv("HOME")) + "/plugins") {
